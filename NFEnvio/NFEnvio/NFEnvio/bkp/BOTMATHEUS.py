...
class Bot(DesktopBot):
    def action(self, execution=None):
        ...
        # Ativa o 'CERTIFICADO_DIGITAL '
        if not self.find("CERTIFICADO_DIGITAL", matching=0.97, waiting_time=10000):
            self.not_found("CERTIFICADO_DIGITAL")
        self.click()

        # Valida o certificado
        if not self.find("certificadoOK", matching=0.97, waiting_time=1000000):
        ...
        if not self.find("trocaPerfil", matching=0.97, waiting_time=1000000):
        ...
        # Add CNPJ
        if not self.find("addCnpj", matching=0.97, waiting_time=1000000):
